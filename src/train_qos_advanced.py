...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Advanced QoS ML pipeline")
    ...
    parser.add_argument("--feature-importance-out", type=Path, default=None)
    parser.add_argument("--timestamped", action="store_true", help="Append UTC timestamp to outputs")
...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["load_factor"] = df["throughput"] / (df["response_time"].replace(0, np.nan))
...
def compute_holdout_metrics(model: Any, X: np.ndarray, y: np.ndarray, cv: KFold) -> Dict[str, float]:
...
def plot_feature_importance(model: Any, feature_names: List[str], out_path: Path) -> Optional[Path]:
    importances = None
...
def main() -> None:
    ...
    filtered = remove_outliers(engineered)

    feature_cols = get_feature_columns(filtered)
    X = filtered[feature_cols].to_numpy()
    y = filtered[TARGET_COLUMN].to_numpy()
    ...
    ensemble.fit(X, y)
    holdout_metrics = compute_holdout_metrics(ensemble, X, y, cv)
    ...
    feature_importance_out = args.feature_importance_out or output_dir / f"feature_importance{suffix}.png"

    feature_importance_path = plot_feature_importance(top_models[0], feature_cols, feature_importance_out)
